  ...
  public:
    ...
    void SetNewValue(G4UIcommand *command, G4String newValues) {
	  ...
	  int iRep = volumeStore->at(i)->GetCopyNo();
        ...
        G4double value;
        G4String unit;
        iss >> value >> unit; // This will read "1.0" into value and "um" into unit

        // Combine value and unit with the multiplication symbol, if necessary for parsing later
        // valueWithUnit = G4String(std::to_string(value)) + "*" + unit;
        valueWithUnit = G4String(std::to_string(value)) + unit;

        // Check if there's another argument for the volume name regex
        std::getline(iss >> std::ws, volNameRegex); // Use std::ws to skip leading whitespace
        printf("stepLimitWithUnit is %s\n", valueWithUnit.c_str());
        printf("volNameRegex is %s\n", volNameRegex.c_str());
        // Now, apply the step limit using the parsed values
        ApplyStepLimit(valueWithUnit, volNameRegex);
      }
      // else if(command == fSetStepLimitCmd) {
      //     G4String stepLimitWithUnit, volNameRegex;
      //     istringstream iss(newValues);
      //     getline(iss, stepLimitWithUnit, ' '); // Assuming space as delimiter
      //     getline(iss, volNameRegex); // Rest of the line is treated as the regex
      //     // Now, apply the step limit using the parsed values
      //     printf("stepLimitWithUnit is %s\n", stepLimitWithUnit.c_str());
      //     printf("volNameRegex is %s\n", volNameRegex.c_str());
      //     ApplyStepLimit(stepLimitWithUnit, volNameRegex);
      // }
    }

    // void ApplyStepLimit(G4double length, const std::string& volNameRegex) {
    //   G4PhysicalVolumeStore* volumeStore = G4PhysicalVolumeStore::GetInstance();
    //   regex pattern(volNameRegex);
    //   bool applyToAll = volNameRegex.empty() || volNameRegex == ".*"; // Apply to all if regex is empty or explicitly to match anything
    //   for (size_t i = 0; i < volumeStore->size(); ++i) {
    //     G4String volName = volumeStore->at(i)->GetName();
    //     if (applyToAll || regex_match(volName, pattern)) {
    //       G4LogicalVolume* logicalVolume = volumeStore->at(i)->GetLogicalVolume();
    //       G4UserLimits* userLimits = new G4UserLimits();
    //       userLimits->SetMaxAllowedStep(length);
    //       logicalVolume->SetUserLimits(userLimits);
    //       G4cout << "Set step limit of " << length / CLHEP::mm << " mm for volume " << volName << G4endl;
    //     }
    //   }
    // }
    ...
    void ApplyStepLimit(const G4String& lengthAndUnit, const G4String& volNameRegex) {
      ...
      // Convert the unit to Geant4 internal units (mm)
      // G4double ustepMax = length * G4UnitDefinition::unit;
      printf("G4double is %f\n", g4_step_max);
      ...
      for (auto* vol : *volumeStore) {
          if (std::regex_match(vol->GetName(), pattern)) {
              ...
              logicalVolume->GetUserLimits()->SetMaxAllowedStep(g4_step_max); // Convert to Geant4's internal units if needed
